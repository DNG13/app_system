...
@section('title', 'Преса всі заявки')

@section('content')
    <div class="row">
        <div class="col-md-10 col-md-offset-1">
            <h4><strong>Заявка преса</strong></h4>
            ...
            <div style="display: inline-block; margin-top: 5px;">
                <form action="{{url('/press')}}" method="GET">
                    <div class="input-group">
                        <input type="text" class="form-control" name="search" placeholder="Пошук(Контактна особа, місто, Назва ЗМІ)" >
            ...
            @if(!count($applications)==0)
                ...
                <table class="table table-striped" border="1" style="margin-top: 5px;">
                    ...
                    @foreach($applications as $application)
                        <tr class="odd" style="{{
                        $application->status == \App\Models\AppPress::APP_STATUS_REJECTED ? 'background-color: #f5ebee' :
                        ($application->status == \App\Models\AppPress::APP_STATUS_ACCEPTED ? 'background-color: #dcedc8' :
                        ($application->status == \App\Models\AppPress::APP_STATUS_WAIT_USER ? 'background-color: #f5f5c4' :
                        ($application->status == \App\Models\AppPress::APP_STATUS_CHANGED ? 'background-color: #e1f5f5' : '')))
                        }}">
                            ...
                            <td><div class="btn-group">
                                    <a class="btn btn-info btn-sm" href="/press/{{$application->id }}" title="Детальніше" >
                                        <i class="fa fa-file-text" aria-hidden="true"></i>
                                    </a>
                                    <a class="btn btn-info btn-sm" href="/press/{{$application->id }}/edit" title="Редагувати">
